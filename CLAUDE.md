...
- `biolit/parsers/utils.py` — `select_sections(secs, sections_wanted, max_tokens)` truncates parsed JATS sections to a char budget; promotes small back-matter sections (data availability/notes/footnotes/supplementary material) and Methods/Materials-and-Methods sections to the front so they survive truncation ahead of narrative sections — needed for journals (e.g. Disease Models & Mechanisms) that place Methods last, after Results/Discussion. A two-pass budget fill keeps everything that fits whole first, then spends any leftover truncating one more section, so one long overflowing section no longer blocks a smaller later section from being included. `max_tokens=None` disables the budget entirely, for non-LLM callers (e.g. exact-string accession search) that need the full text rather than a truncated excerpt
...
- `biolit/parsers/jats.py` — JATS text extraction: `\n` at block boundaries (`sec`, `p`, `table`/`tr`/`td`, `fn`, `notes`, …), none at inline boundaries (`sup`, `sub`, `xref`, …) so markup-split compound terms stay glued. `parse_jats_sections` keys top-level `<sec>` only (nested subsections dropped as duplicates) plus a `footnotes` key from `<fn>` and a `notes` key from `<notes>` (data-availability/funding/ethics statements published outside a body `<sec>`). `<sec>` elements nested inside `<abstract>` (structured-abstract subsections) are excluded from body-section extraction so they can't shadow the paper's real Methods/Results
- `tests/` — pytest suite
